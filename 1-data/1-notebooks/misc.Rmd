...
```{r misc-setup, echo = FALSE, warning=FALSE,message=FALSE}
library(knitr)
library(rprojroot)
library(tidyverse)
library(rgdal)
...
if(!file.exists(root_file('1-data/3-external/seattle.gpkg'))){
        tigris::places(state = "WA") %>%
        tigris::filter_place(place = "Seattle") %>%
        spTransform(CRSobj = crs_proj) %>% 
        writeOGR(dsn = root_file('1-data/3-external/seattle.gpkg'),
                 layer = "seattle", driver = "GPKG", verbose = FALSE, 
                 overwrite_layer = TRUE)
}

sea <- readOGR(dsn = root_file('1-data/3-external/seattle.gpkg'),layer = "seattle",verbose = FALSE)
...
if(!file.exists(root_file('1-data/3-external/seattle_ccd.gpkg'))){
        tigris::county_subdivisions(state = "53",county = "033") %>% 
                subset(NAME == "Seattle") %>% 
                spTransform(CRSobj = crs_proj) %>% 
                writeOGR(dsn = root_file('1-data/3-external/seattle_ccd.gpkg'),
                         layer = "seattle", driver = "GPKG", verbose = FALSE, 
                         overwrite_layer = TRUE)
}

sea_ccd <- readOGR(dsn = root_file('1-data/3-external/seattle_ccd.gpkg'),layer = "seattle",verbose = FALSE)
...
if(!file.exists(root_file('1-data/4-interim/tr-ccd-sp.gpkg'))){
        tr_ccd_geoid <- 
        read_csv(
                root_file('1-data/3-external/DEC_10_SF1_H10_tr_sea_ccd/DEC_10_SF1_H10_with_ann.csv'), 
...
wtr_sp <-
        readOGR(dsn = fp_wtr,      # create a waterbodies shape
