...
pu_no_condo_tbl <- read_rds(root_file('1-data/4-interim/prcl-presuse-no-condo.rds')) %>% 
        kable(col.names = c('CODE',
                            'DESCRIPTION',
                            'ASSUMED UNIT COUNT'))

pu_incl_condo_tbl <- read_rds(root_file('1-data/4-interim/prcl-presuse-incl-condo.rds')) %>% 
        kable(col.names = c('CODE',
                            'DESCRIPTION',
                            'ASSUMED UNIT COUNT'))
...
The following categories of "Present Use" (a classification used by the King County assessor) are used to distinguish these two subsets of the parcel data:
