...
```{r misc-tracts-ccd-wtr}


if(!file.exists(root_file('1-data/4-interim/tr-ccd-wtr-sf.rds'))){
        tr_ccd_geoid <- 
                read_csv(
...
subset(tr, GEOID %in% tr_ccd_geoid) %>% 
        ...
        write_rds(root_file('1-data/4-interim/tr-ccd-wtr-sf.rds'))
}

tr_ccd_wtr_sf <- read_rds(root_file('1-data/4-interim/tr-ccd-wtr-sf.rds'))

show_tr_ccd_sf <- function(){
        myLfltGrey() %>% 
        addPolygons(data = as(tr_ccd_wtr_sf,"Spatial"),weight = .5,color = blue,opacity = 1,fillColor = blue,fillOpacity = .5) %>% 
...
wtr_sf %>% 
        ...
        wtr_sf[.,] %>% 
        st_union %>% 
        st_intersection(st_union(sea_ccd_sf)) %>% 
...
### Tracts in Seattle CCD Without Waterbodies
