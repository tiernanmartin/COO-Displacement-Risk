...
if(!file.exists(root_file('1-data/4-interim/kc-sf.rds'))){
        kc_noclip_sf %>% 
                mutate(geom = st_difference(geom, st_union(wtr_kc_sf))) %>% 
...
if(!file.exists(root_file('./1-data/4-interim/coo-acs-2015-geoms-sf.rds'))){
        list(
                kc_sf %>% 
                        select(geom) %>% mutate(NAME = 'KC', NAME_FULL = 'King County, Washington',GEOID6 = NA_character_, SEACCD_LGL = NA),
                sea_ccd_sf %>% 
                        select(geom) %>% mutate(NAME = 'SEACCD', NAME_FULL = 'Seattle CCD, King County, Washington', GEOID6 = NA_character_, SEACCD_LGL = NA),
                tr_kc_2015_sf %>% 
                        select(NAME,NAME_FULL = NAMELSAD,GEOID6 = GEOID,SEACCD_LGL,geom) %>% mutate(NAME_FULL = paste0(NAME_FULL,', King County, Washington'),GEOID6 = str_sub(GEOID6,6,11))
        ) %>% 
                reduce(rbind.sf) %>%
...
if(!file.exists(root_file('./1-data/4-interim/coo-acs-2009-geoms-sf.rds'))){
        list(
                ...
                sea_ccd_sf %>% 
                        select(geom) %>% mutate(NAME = 'SEACCD', NAME_FULL = 'Seattle CCD, King County, Washington', GEOID6 = NA_character_, SEACCD_LGL = NA),
                tr_kc_2009_sf %>% 
                        select(NAME = NAME00, NAME_FULL = NAMELSAD00,GEOID6 = TRACTCE00,SEACCD_LGL,geom) %>% mutate(NAME_FULL = paste0(NAME_FULL,', King County, Washington'))
        ) %>% 
                reduce(rbind.sf) %>% 
