...
currency_rates <- 
        list(1,currency_convert_rate(2015,2004),currency_convert_rate(2015,1999)) %>% 
        set_names(c(2015,2004,1999))

# Remove duplicates, join the three datasets, convert the currency to 2015 dollars and save
...
list(
        ...
        mutate(RATE = map_dbl(YEAR, ~ .x %>% as.character %>% extract2(currency_rates,.))) %>% 
        mutate(APPRLANDVAL = multiply_by(APPRLANDVAL,RATE) %>% round_any(1),
               APPRIMPSVAL = multiply_by(APPRIMPSVAL,RATE) %>% round_any(1)) %>% 
        select(-BILLYR) %>% 
        arrange(PIN) %>% 
