...
printable_acs_sf <- function(acs_sf){
        is_type <- function(x) x %>% extract2(1) %>% typeof %!in% c('S4','list')

        acs_sf %>% 
                unclass() %>% 
                as_tibble() %>% 
                mutate_if(is_type,unlist)
}

acs_2009_sf %>% printable_acs_sf
```


#### Census Tables, 2011-2015
```{r census-show-2010-table}
acs_2015_sf %>% printable_acs_sf
```
...
acs_2009_sf %>% select(NAME_FULL,GEOID6,ENDYEAR,B03002_003) %>% slice(3) %>% printable_acs_sf
```
...
acs_2015_sf %>% 
        ...
        map(.f = normalize_acs,
            ...
            tables = tables) %>% 
        reduce(bind_rows) %>% 
        left_join(acs_2015_sf_renamed,.,by = c('GEOID6' = 'GEOID6_2009_NORML')) %>% 
